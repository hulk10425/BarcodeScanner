...
open class BarcodeScannerController: UIViewController,UITableViewDelegate, UITableViewDataSource {
    ...
    lazy var cartButton: UIButton = { [unowned self] in
        let button = UIButton(type: .custom)
        button.addTarget(self, action: #selector(creatTableView), for: .touchUpInside)
        return button
        }()
    ...
    func creatTableView() {
        ...
        // let table: UITableViewController = CartTableViewController()
        
        let tableView: UITableView = UITableView()
        tableView.frame = CGRect(x: 10, y: 10, width: 100, height: 500)
        tableView.frame = CGRect(x: 10, y: 180, width:view.frame.width - 40, height: view.frame.height/2)
